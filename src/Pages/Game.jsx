/* eslint-env es6 */
/* codacy ignore ESLint8_es-x_no-modules */
/* codacy ignore ESLint8_es-x_no-destructuring */
/* codacy ignore ESLint8_es-x_no-block-scoped-variables */
/* codacy ignore ESLint8_es-x_no-async-functions */
/* codacy ignore ESLint8_n_no-missing-import */
...
export default function GamePage() {
  ...
  const calculateScore = useCallback((ballPositions) => {
    ...
    const centerBallInRing =
      Math.sqrt(
        ballPositions.center_ball.x ** 2 + ballPositions.center_ball.z ** 2,
      ) <= centerRingRadius;
    ...
    const getBallData = (balls, player) =>
      balls
        ...
        .map((ball) => ({
          ...ball,
          distance: Math.sqrt(
            (ball.x - ballPositions.center_ball.x) ** 2 +
              (ball.z - ballPositions.center_ball.z) ** 2,
          ),
    ...
    allBalls.forEach((ball) => {
      ...
      if (
        touchingCenter &&
        ball.distanceFromCenter <= centerRingRadius &&
        centerBallInRing
      ) {
    ...
    const ballsOutsideRing = allBalls.filter(
      (ball) => ball.distanceFromCenter > centerRingRadius,
    );
  ...
  const handleShoot = useCallback(
    async ({ angle, power }) => {
      ...
      try {
        const updatedBallPositions = JSON.parse(
          JSON.stringify(currentGame.ball_positions),
        );
        const isPlayer1 = selectedBall.isPlayer1;

        const ballList = isPlayer1
          ? updatedBallPositions.player1_balls
          : updatedBallPositions.player2_balls;
        ...
        const nextPlayer =
          nextTurn === 1 ? currentGame.player1_name : currentGame.player2_name;

        const allPlayer1BallsPlayed = updatedBallPositions.player1_balls.every(
          (b) => !b.active,
        );
        const allPlayer2BallsPlayed = updatedBallPositions.player2_balls.every(
          (b) => !b.active,
        );
        ...
        const gameEnded =
          scores.player1 >= currentGame.target_score ||
          scores.player2 >= currentGame.target_score;
        const winner = gameEnded
          ? scores.player1 >= currentGame.target_score
            ? currentGame.player1_name
            : currentGame.player2_name
          : null;
      ...
      <div className="flex-grow flex flex-col gap-4">
        ...
        <div className="w-full glass-panel rounded-2xl p-4 flex flex-col">
          ...
          <div className="mt-3 pt-3 border-t border-slate-700">
            ...
            <p className="text-center text-cyan-300 font-medium h-12 flex items-center justify-center">
              {gameMessage}
            </p>
          </div>
        </div>
